...
transformed parameters {
...

    {
        for (i in 1:R) {
            ...
            beta_hla[i] = z[i] .* lambda_tilde[i] * tau[i] + (Z[i] * epitope_effect);
...
generated quantities {
    ...
    for (i in 1:R) {
        ...
        for (n in 1:samples) {
            ...
            x_i = to_vector(xs[i, (2 + (n - 1) * (D + 1)):(n * (D + 1))])';
            phylogeny = xs[i, 1 + (n - 1) * (D + 1)];

            y = ys[i, n + 1];

            log_lik[i][n] = bernoulli_logit_lpmf(y | beta[1] * logit(phylogeny) + 
                intercepts[i] + x_i * beta_hla[i]);
        }
    }
}