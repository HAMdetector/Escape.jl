...
@testset "thetas(::Model4Result, ::Int, ::Int)" begin
    ds = Escape.HLADataset("Test")
    data = Escape.stan_input(Escape.Model4(), ds.data[1], mincount = 2)

    sf = @suppress Escape.stan(
        joinpath(@__DIR__, "stan", "model_4_gq"), data,
    ...
    res = Escape.Model4Result(sf, alleles, r)
...
@testset "pointwise_loglikelihoods(::Model4Result, ::Int, ::Int)" begin
    ds = Escape.HLADataset("Test")
    data = Escape.stan_input(Escape.Model4(), ds.data[1], mincount = 2)
