...
Our general approach is to fit a Bayesian model that captures relationships between host HLA alleles and replacements in viral proteomes. This approach is advantageous because it improves estimates by including prior information (even if only knowledge of effect magnitudes), adding problem-specific structure and partial pooling~\cite{Gelman2010}.

Among the novel features of our method that markedly improve detection of HLA-associated mutations (HAMs) are the inclusion of additional relevant information and the use of sparsity-inducing priors, as described in the following. 
...
\subsection*{Sparsity-inducing priors}
  ...
  \textit{Posterior predictive checks}
  ...
  \textit{Leave-one-out cross-validation}
...
\begin{align}
  ...
  \theta_{ik} & = \text{logit}^{-1}(\beta_{0_{k}} + \sum_{j=1}^{D} X_{ij}\beta_{\text{HLA}_{jk}}),
\end{align}
