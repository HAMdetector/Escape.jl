...
\hl{DaHo: LOO auch in Methods einführen und kürzen}
...
Perhaps surprisingly, the model with epitope prediction performs roughly as well as the model without. This is true from a perspective of predictive accuracy. However, in determining which HLA alleles are associated with a given replacement, adding epitope prediction is highly useful, as shown in the previous section. This observation is explained by the circumstance that leave-one-out cross-validation takes all positions and replacements into account, but a given HLA allele is only restricting a subset of all positions in a sequence alignment. Additionally, the benefit of accurately identifying an associated HLA allele is only relevant for the samples that do have that allele, which for most alleles is only a small subset of all available samples. Therefore, looking at predictive accuracy alone is misleading in that regard, because we are more interested in the predictors than in the predictions themselves. \hl{DaHo: diesen Abschnitt etwas klarer schreiben}

\subsection{A real-world example}

We had the opportunity to test our model against a set of associations identified by Fisher's exact test that turned out to be false positives \hl{DaHo: diesen Abschnitt  glattziehen}. A comparison with a preliminary version of HAMdetector have been published recently. Here we provide a comparison between results from Fisher's exact test~\cite{Budeus2016} and the mature version of HAMdetector used in this work.
...
\hl{DaHo: wir haben zwar mehrere Datensätze, gehen aber praktisch nicht auf die Resultate mit diesen Datensätzen ein. Wie besprochen sollte dieses Manuskript eher methodisch sein, d.h.~es ist ok, dass wir nicht *viel* auf die virologische/immunologische Seite eingehen. Aber, dass wir überhaupt nichts zu den Resultaten sagen, ist etwas zu wenig. Z.B. könnte man die Vielfalt der Datensätze nutzen um den Punkt zu machen, dass die Verbesserungen nicht von den Details des Virus abhängen. Vielleicht fällt Dir ja noch was dazu ein.}
