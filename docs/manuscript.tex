...
Epitope prediction is information about the expected degree of sparsity, i.e. when knowing a certain position is restricted by a certain HLA allele, we expect that this HLA allele is more likely to be associated with the replacement than the other HLA alleles.
This can be reflected by increasing the scale of the local shrinkage parameters \(\lambda_{j}\):

\begin{equation}
  \begin{aligned}
    \lambda_{j} &\sim \text{Cauchy}^{+}(0, \text{exp}(\beta_{\text{epi}}Z_{j})) \\
    \beta_{\text{epi}} &\sim \text{Normal}^{+}(1, 2)
  \end{aligned}
\end{equation},
 
where \(Z_{j}\) is a binary vector denoting which of the alleles is predicted to restrict HLA binding or antigen processing at the alignment position corresponding to replacement \(j\).
The parameter \(\beta_{\text{epi}}\) governs the increase in scale of the local corresponding local shrinkage parameters. The larger \(\beta_{\text{epi}}\) is, the more likely it is that we expect to see non-zero regression coefficients for the HLA alleles that are predicted to restrict the corresponding alignment position of replacement \(j\).
