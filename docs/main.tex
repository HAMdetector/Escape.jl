...
\begin{methods}
  ...
  Our general approach for HAMdetector is to fit Bayesian regression models that captures relationships between host HLA alleles and replacements in viral proteomes.
  
  This Bayesian approach is advantageous because it allows use of: (1) prior information (e.g.\ knowledge of effect magnitudes), (2) relevant additional information (phylogeny, epitope information), (3) a problem-specific structure, (4) partial pooling~\citep{Gelman2010}.
...
For a first posterior predictive check we used \emph{calibration plots} (Fig. \ref{fig:calibration}): two binned quantities were plotted against each other, the observed relative frequencies of replacements $f(y_{ik}=1)$, and the predicted probabilities $P(y_{ik}=1|\text{model})$. In such a plot, a well-calibrated model should yield points following the diagonal. Technically, all observations were first sorted by increasing estimated probability $P(y_{ik}=1|\text{model})$ and grouped into $n$ bins. For each bin, the fraction of observations with $y_{ik}=1$ (observed event percentage) was then plotted against the midpoint of each bin. The cutpoints of the bins are indicated by error bars.
...
\begin{figure}
 \includegraphics[width=1.0\linewidth]{plots/calibration_plot.pdf}  
  \caption{Calibration plot for the HBe antigen.}
...
The epitope and processing predictions that HAMdetector uses are imperfect, as the underlying tools extrapolate binding affinities for new epitopes based on experimental data. Bayesian statistics provides a coherent framework to make use of imperfect data. In HAMdetector, this is achieved by an additional parameter \(\beta_{\text{epi}}\) that governs how strongly the model takes an apparent association between a replacement and the corresponding HLA allele into account. By default, the regression coefficients that quantify the strength of association between allele and replacement are shrunk towards 0, and only in the presence of considerable evidence in favor of an association (e.g. because the replacement often co-occurs with a certain HLA allele), this shrinkage is overcome by the observed data.

If the epitope prediction happens to be reliable, i.e. when the presence of a predicted epitope correlates strongly with the probability observing the replacement in a host with the respective HLA allele, the parameter \(\beta_{\text{epi}}\) is estimated to be large and less evidence by the sequence data is enough to escape the shrinkage and estimate a non-zero association between allele and replacement, compared to associations that do not lie inside a predicted epitope.
Likewise, if the epitope prediction turns out to be non-reliable, \(\beta_{\text{epi}}\) is estimated to be close to 0 and the presence of a predicted epitope does not strongly affect the conclusions drawn from the sequence data.

However, it is important to consider that biologically relevant HLA escape mutations do not necessarily have to lie within or close to the boundary of an epitope. Compensatory mutations \citep{Kelleher2001, Ruhl2011, NeumannHaefelin2011} can confer a strong selective advantage, e.g. by partially restoring replicative capacity that would otherwise be impaired by the exclusive presence of a certain HLA escape mutation. As they also facilitate HLA escape and are associated with HLA alleles in a sequence alignment, they can also be considered as HLA escape mutations.
In a sequence alignment, compensatory mutations can occur far away from the epitope they are associated with, as they might be the result of improved physical interactions with another amino acid in the folded, three-dimensional protein.

We therefore also expect HAMs outside epitopes and one possible concern is that the model focuses too strongly on associations with replacements that lie within the boundary of predicted epitopes.

\begin{figure}
  \centering
  \includegraphics[width=0.95\linewidth]{plots/m3_m4_HBe.pdf}
  \caption{Integral of the marginal posterior $P(\beta_{jk}>0)$ for the HAMdetector model with epitope prediction (model 4) and without epitope prediction (model 3).}
  \label{fig:m3m4}
\end{figure}

Figure~\ref{fig:m3m4} shows posterior probabilities \(P(\beta_{jk}>0)\) for replacement--HLA allele pairs as calculated by HAMdetector with (model 4) and without (model 3) epitope prediction. Each replacement--HLA allele pair is represented by a dot and colored according to whether or not that position lies within a predicted epitope.
For replacements that do not lie within a predicted epitope, both models provide similar estimates (points across the diagonal). However, some replacements--HLA pairs that have only weak evidence of association in model 3 have strong support in model 4, which is explained by the additional evidence provided by epitope prediction.
The figure shows that the model is still able to identify associations outside predicted epitopes and that epitope information augments evidence obtained from sequence data. 
