// like model 4, but without epitope prediction and phylogeny
    ...
    vector ll(vector global_pars, vector local_pars, real[] xs, int[] ys) {
...

        {
            ...
            lp += bernoulli_logit_glm_lpmf(segment(ys, 3 + S, S)[segment(ys, 3, y_counts)] | 
                to_matrix(segment(xs, 2, S * D), S, D)[segment(ys, 3, y_counts)], 
                b0_hla, beta_hla);
        }
...
data {
    ...
    int rs[N];
    int idx[N];
    matrix[R, S] phy; // unused in model 2
    matrix[R, D] Z; // unused in model 2
}
