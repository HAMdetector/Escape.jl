...
functions {
    ...
    real testew(real[] x) {
        real p = mean(x);
        real H = -1 * p * log2(p) - (1 - p) * log2(1 - p);
    ...
    vector ll(vector global_pars, vector local_pars, real[] xs, int[] ys) {
        ...
        real lambda_0 = global_pars[2];
        real b_var = global_pars[3];
        ...
        {   
            ...
            vector[D] lambda = (aux1_lambda .* sqrt(aux2_lambda)) .* 
                (exp(b_var) * sqrt(to_vector(segment(xs, 2 + D + S + S * D, D)))) .* 
                exp(lambda_0 + b_epi * to_vector(segment(xs, 2, D)));
...
model {
...

    {
        ...
        vector[3] global_pars;

        global_pars[1] = b_epi;
        global_pars[2] = lambda_0;
        global_pars[3] = b_var;
...
generated quantities {
    ...
    for (r in 1:R) {
        ...
        vector[D] lambda = (aux1_lambda[r] .* sqrt(aux2_lambda[r])) .*
            (exp(b_var) * sqrt(s_j_sq)) .* exp(lambda_0 + b_epi * to_vector(Z[r,]));
